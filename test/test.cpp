...
public:
  ...
  // to set low.
  PinResponder(byte pinC, byte pinR) : DataStreamObserver(false, false) {
  ...
  ~PinResponder() { state->digitalPin[pinC].removeObserver(observer); }
  // when we get a change on a column, we send the same change on the row to
  // indicate that the circuit is closed (button is pressed)
  virtual void onBit(bool aBit) { state->digitalPin[pinR] = aBit; }
...
void testKey(byte column, byte row, char key, Keypad keypad) {
  ...
  delay(11); // the library defines a debounce delay of 10ms before recognizing
             // any activity
